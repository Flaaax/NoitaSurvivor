...
namespace flx::ui {
	void Tooltip::updateLayout() {
		...
		if (sprite) {
			const rect bounds = sprite->getGlobalBounds();
			layout += bounds.size;
			if (!textLines.empty() && hasTitle) {
				auto& title = textLines.front();
				title.setPosition({layout.x, 0});
				const vec2 titleLayout = title.getLayoutSize();
		...
		for (auto& text : textLines) {
			...
			const vec2 textLayout = text.getLayoutSize();
	...
	void Tooltip::draw(const UIPainter& canvas) const {
