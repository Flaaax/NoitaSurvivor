...
namespace flx::ui {
	...
	text::TextStyle NRichTextShape::getDefaultStyle() const {
		return m_defaultStyle;
	}

	void NRichTextShape::setDefaultStyle(text::TextStyle style) {
	...
	vec2 NRichTextShape::getLayoutSize() const {
		rebuildCache();
		return m_layoutSize;
	}

	rect NRichTextShape::getGlobalLayout() const {
	...
	void NRichTextShape::appendRun(u64 byteBegin, u64 byteEnd, text::TextStyle style) const {
	...
	nquad NRichTextShape::applyStyleToQuad(nquad quad, text::TextStyle style, u64 glyphIndex) const {
		nquad ret = quad;
	...
	void NRichTextShape::appendQuad(flx::Vector<sf::Vertex>& vertices, nquad quad, rect textureRect, sf::Color color) {
