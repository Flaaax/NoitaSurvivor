#include "NRoundRectShape.h"
#include "src/utils/Math.h"

void NRoundRectShape::updatePoints() {
	m_points.resize(m_segments * 4);

	size_t index = 0;
	const float step = (Util::PI / 2.f) / static_cast<float>(m_segments - 1);
	for (size_t corner = 0; corner < 4; corner++) {
		...
		case 0:
			...
			cornerEnd = cornerPos + nvec2{0, m_radius};
			break;
		case 1:
			cornerPos = {m_radius, m_size.y - m_radius};
			cornerEnd = cornerPos + nvec2{-m_radius, 0};
			break;
		case 2:
		...
		for (size_t i = 0; i < m_segments - 1; i++) {
			const float angle = corner * (Util::PI / 2.f) + i * step;
			const float x = m_radius * std::cos(angle);
			const float y = m_radius * std::sin(angle);
			m_points[index++] = cornerPos + nvec2{x, y};
		}
