...
public:
	...
	template<class ...Args>
	NString(const std::format_string<Args...> fmt, Args&&... args) : m_data(std::format(fmt, std::forward<Args>(args)...)) {}
	...
	NString operator+(const NString& other)const {
		return NString(m_data + other.m_data);
	}
