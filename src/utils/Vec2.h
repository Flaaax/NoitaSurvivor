...
struct vec2_base {
	...
	constexpr vec2_base(const _vec& vec) noexcept
	...
	template <vec2_t<T> _vec>
	[[nodiscard]] _vec& as() noexcept {
		return *reinterpret_cast<_vec*>(this);
	}

	template <vec2_t<T> _vec>
	[[nodiscard]] const _vec& as() const noexcept {
		return *reinterpret_cast<const _vec*>(this);
	}
};
...
struct nvec2 : public Util::internal::vec2_base<nvec2, float> {
	...
	operator fvec2&() noexcept {
		return as<fvec2>();
	}
	operator const fvec2&() const noexcept {
		return as<fvec2>();
	}
	operator bvec2&() noexcept {
		return as<bvec2>();
	}
	operator const bvec2&() const noexcept {
		return as<bvec2>();
	}

	[[nodiscard]] float length() const noexcept {
		return ::nmath::c_sqrt(x * x + y * y);
	}
	...
	[[nodiscard]] bool isValid() const noexcept {
		return ::nmath::c_isvalid(x) && ::nmath::c_isvalid(y);
	}
...
struct nvec2u : public Util::internal::vec2_base<nvec2u, unsigned int> {
	...
	operator sf::Vector2u&() noexcept {
		return as<sf::Vector2u>();
	}

	operator const sf::Vector2u&() const noexcept {
		return as<sf::Vector2u>();
	}
};
...
struct nrect {
	...
	operator frect&() {
		return *reinterpret_cast<frect*>(this);
	}
	operator const frect&() const {
		return *reinterpret_cast<const frect*>(this);
	}
};
