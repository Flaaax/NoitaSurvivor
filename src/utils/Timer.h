...
public:
	...
	CTimer& set(float duration, Func&& onTimeout = {}) {
		this->m_duration = Util::max(duration, MIN_DURATION);
	...
	N_NODISCARD bool isRunning()const { return this->m_isRunning; }
	N_NODISCARD float getRemainingTime()const { return this->m_remainingTime; }
};
...
public:
	...
	Timer(Timer&& other) noexcept :Timer(other) { other.clear(); }
	...
	Timer& set(float duration) {
		m_duration = Util::max(duration, MIN_DURATION);
		return *this;
	}
	...
	N_NODISCARD bool isRunning()const { return this->m_isRunning; }
	N_NODISCARD float remainingTime()const { return this->m_remainingTime; }
	N_NODISCARD float getProgress()const { return 1.f - m_remainingTime / m_duration; }
};