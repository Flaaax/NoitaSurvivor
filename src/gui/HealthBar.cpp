...
HealthBar::HealthBar(const nvec2& topRight, const nvec2& size, int initialMaxHealth, float lengthPerHealth, int mode)
	: m_initialHealth(initialMaxHealth), lengthPerHealth(lengthPerHealth),
	m_initialLength(size.x), m_right(topRight.x), mode(mode) {
...
void HealthBar::updateText() {
	if (mode == 1) {
		text.setString(NString("µÈ¼¶{}", level));
	}
	else if (mode == 0) {
...
void HealthBar::draw(Renderer& renderer) const {
	...
	fvec2 healthShapeSize = { (m_health / (float)m_maxHealth) * m_geometry.w , m_geometry.h };
	...
	healthBarShape.setPosition(posRect.position.as_fvec2);
	nrect backGroundRect({ 0,0 }, m_geometry.size + nvec2{ 8,8 });
	backGroundRect.setCenter(posRect.center());
	backGroundShape.setSize(backGroundRect.size.as_fvec2);
	backGroundShape.setPosition(backGroundRect.position.as_fvec2);
