...
void PhysicsService::assertValid(const BodyComponent& bc) {
	if (!b2Body_IsValid(bc.body) || !b2Shape_IsValid(bc.shape)) {
		LoggerOld::error_and_throw("Body or shape is invalid");
	}
}

void PhysicsService::assertWorldMutable(const GameCtx& ctx) {
	if (ctx.worldCtx.isStepping) {
		LoggerOld::error_and_throw("World is currently not mutable");
	}
}
...
void PhysicsService::createBody(const GameCtx& ctx, myecs::entity e, const BodyArg& arg) {
	...
	if (!bc) {
		LoggerOld::warn("A BodyComponent is required to create a physical body!");
		return;
	}
	if (b2Body_IsValid(bc->body)) {
		LoggerOld::warn("Body of entity {} already exists!", e.string());
		return;
	}
	if (ctx.worldCtx.isStepping) {
		LoggerOld::warn("Cannot create body when world is stepping!");
		return;
	}

	const auto ec = ctx.reg.try_get<EntityComponent>(e);
	if (!ec) {
		LoggerOld::error_and_throw("Entity must have EntityComponent to create a body");
	}
	...
	if (arg.shape == BodyArg::Box) {
		...
		LoggerOld::warn("Shape type not supported: {}", static_cast<int>(arg.shape));
	}
}
...
float PhysicsService::getRadius(const BodyComponent& bc) {
	...
	default:
		LoggerOld::error_and_throw("Shape type {} not supported", static_cast<int>(type));
	}
...
void PhysicsService::applySoftCollision(const GameCtx& ctx, myecs::entity a, myecs::entity b) {
	if (a == b) {
		LoggerOld::warn("PhysicsService::applySoftCollision does not accept same entities");
		return;
	}
