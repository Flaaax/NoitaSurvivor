...
namespace flx::game {
	...
	vec2 PhysicsService::getPosition(const GameCtx& ctx, myecs::entity e) {
		return getPosition(getBody(ctx, e));
	}

	vec2 PhysicsService::getPosition(const BodyComponent& bc) {
		assertValid(bc);
		return b2Body_GetPosition(bc.body);
	}

	void PhysicsService::setPosition(const GameCtx& ctx, myecs::entity e, vec2 position) {
		setPosition(getBody(ctx, e), position);
	}

	void PhysicsService::setTransform(const BodyComponent& bc, vec2 position, float rad) {
		assertValid(bc);
	...
	void PhysicsService::setPosition(const BodyComponent& bc, vec2 position) {
		assertValid(bc);
		const auto rot = b2Body_GetRotation(bc.body);
		b2Body_SetTransform(bc.body, position, rot);
	}

	vec2 PhysicsService::getVelocity(const BodyComponent& bc) {
		assertValid(bc);
		return b2Body_GetLinearVelocity(bc.body);
	}

	vec2 PhysicsService::getVelocity(const GameCtx& ctx, myecs::entity e) {
		return getVelocity(getBody(ctx, e));
	}

	void PhysicsService::setVelocity(const BodyComponent& bc, vec2 velocity) {
		assertValid(bc);
		b2Body_SetLinearVelocity(bc.body, velocity);
	}

	void PhysicsService::setVelocity(const GameCtx& ctx, myecs::entity e, vec2 velocity) {
	...
	void PhysicsService::applyImpulse(const BodyComponent& bc, vec2 impulse) {
		assertValid(bc);
		b2Body_ApplyLinearImpulseToCenter(bc.body, impulse, true);
	}

	void PhysicsService::applyImpulse(const GameCtx& ctx, myecs::entity e, vec2 impulse) {
		return applyImpulse(getBody(ctx, e), impulse);
	}

	void PhysicsService::applyForce(const BodyComponent& bc, vec2 force) {
		assertValid(bc);
	...
	void PhysicsService::applySoftCollision(const GameCtx& ctx, myecs::entity a, myecs::entity b) {
		...
		const vec2 posA = b2Body_GetWorldCenterOfMass(bodyA);
		const vec2 posB = b2Body_GetWorldCenterOfMass(bodyB);

		const vec2 delta = posB - posA;
		...
		const vec2 velA = b2Body_GetLinearVelocity(bodyA);
		const vec2 velB = b2Body_GetLinearVelocity(bodyB);
		...
		const float forceMagnitude = std::max(stiffness * s - damping * normalVelocity, maxForce);

		if (forceMagnitude <= 0.0f)
			return;

		const vec2 force = forceMagnitude * normal;
