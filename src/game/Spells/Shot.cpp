...
namespace flx::game {
	void Shot::modify(const GameCtx& ctx, myecs::entity proj, const Shared<ProjectileSpell>& ps, Span<const Shared<ModifierSpell>> projMods) {
	...
	myecs::entity Shot::fire(const GameCtx& ctx, const Shared<ProjectileSpell>& ps, Span<const Shared<ModifierSpell>> projMods, vec2 shotPos, float arg) {
		...
		const auto p = ps->createProjectile(ctx, shotPos, vec2::rad(arg + rand_scatter));
		modify(ctx, p, ps, projMods);
		return p;
	}

	myecs::entity Shot::clone(const GameCtx& ctx, myecs::entity proj, vec2 shotPos, float arg) {
		const auto& s = ctx.reg.get<ProjectileComponent>(proj);
