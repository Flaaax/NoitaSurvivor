...
public:
	Transition(EffectState beginState, EffectState endState, float duration, float(*easing_function)(float) = Easing::linear) :
		beginState(beginState), endState(endState), easing_function(easing_function) {
	...
	void update(float dt) {
		...
		float w = easing_function(progress);
	...
	void apply(sf::Sprite& sprite)const override {
		...
		color.a = (sf::Uint8)std::floorf(currentState.opacity * color.a);
...
public:
	...
	void update(float dt)override {
		...
		if (!direction)progress = 1 - progress;
