...
private:
	...
	Vector<DenseSet<component, std::identity, std::equal_to<component>, VoidAlloc> >
	entity_components;
	...
	struct poly_view_iterator {
		...
		auto get() {
			return std::tuple<entity, T&, Types&...>(*cur, reg.get<T>(*cur),
			                                         reg.get<Types>(*cur)...);
		}
	};
...
public:
	...
	Registry(Registry&& other) noexcept : pools(std::move(other.pools)),
