...
namespace myecs {
	...
	namespace detail {
		...
		struct VariantVTable {
			...
			template<class T>
			VariantVTable(std::in_place_type_t<T>) {
	...
	private:
		...
		template<class T>
		T& raw() { return detail::raw<T>((void*)data); }
		template<class T>
		const T& raw() const { return detail::raw<T>((void*)data); }

		template<class T>
		static constexpr bool _valid =
			sizeof(T) <= MaxSize
			&& alignof(T) <= Traits.align
			&& Traits.valid<T>();
	public:
		Variant() noexcept {}
		Variant(Variant&& other)noexcept requires (Traits.move) {
		...
		Variant& operator=(Variant&& other)noexcept requires (Traits.move) {
		...
		T& emplace(Args&&... args) requires _valid<T> {
			clear();
		...
		template<class T>
		bool has() const noexcept requires _valid<T> {
			return table == detail::VariantVTable::get<T>();
		}

		template<class T>
		T* try_get() noexcept requires _valid<T> {
			return has<T>() ? &raw<T>() : nullptr;
		}
		template<class T>
		const T* try_get() const noexcept requires _valid<T> {
			return has<T>() ? &raw<T>() : nullptr;
		}

		template<class T>
		T& get() requires _valid<T> {
			T* ptr = try_get<T>();
		...
		template<class T>
		const T& get() const requires _valid<T> {
		...
		template<class T>
		static constexpr bool valid() { return _valid<T>; }

		bool empty() const noexcept { return !table; }

		void* pointer() { return data; }
		const void* pointer()const { return data; }
	};
