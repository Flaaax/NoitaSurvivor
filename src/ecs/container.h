...
namespace myecs {
	...
	namespace detail {
		...
		struct BoolWrapper {
			...
			BoolWrapper& operator=(bool b) noexcept { value = b; return *this; }
			...
			BoolWrapper& operator=(BoolWrapper&& other)noexcept { value = move_and_reset(other.value); }
		};
	...
	private:
		...
		static u32 id(T elem) {
			...
			else return static_cast<u32>(elem);
		}
	public:
		...
		bool has(T elem)const {
			...
			return sparse[i] != null_id
				&& ((!std::is_same_v<T, entity>) || dense[sparse[i]] == elem);		//Check if version matches
		}
	...
	public:
		...
		u64 max_count()const { return sparse.size(); }
		...
		bool full()const { return unused_id.empty(); }
	};
	...
	struct Iterable {
		struct Iter {
			...
			decltype(auto) operator*()const { return it->cur.get(); }
			decltype(auto) operator*() { return it->cur.get(); }
			bool operator==(const Iter& other)const { return stop && other.stop; }
		};
